...
import { Remfot } from '@/app/admin/remfot'
...
const Login = () => {
  const [mbid, setmbid] = useState<string | null>('')
    ...
    useEffect(() => {
      console.log(mbid , mbpass , role)
      if(localStorage.getItem("token") && (localStorage.getItem("role") == "mem" || localStorage.getItem("role") == "sec" ||localStorage.getItem("role") == "jsec") ){
        router.push(`${process.env.NEXT_PUBLIC_HOST}/member`)
      }
    }, [ router])
    ...
    const handleSubmit = async (e)=>{
      ...
      if(res.data.success){
        ...
        await toast.success('Member Found', {
        ...
        await toast.error('Member Not Found', {
            position: "bottom-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
      ...
      <div className="grid lg:grid-cols-2 gap-4 max-lg:gap-12 bg-[url('https://m.media-amazon.com/images/M/MV5BMTJiNjBlYjgtYWVhYy00YWRiLWFkYzEtY2IxNzQ1OWI2NTZkXkEyXkFqcGdeQXVyMTQ4Mjk2NTA2._V1_.jpg')] px-8 py-12 h-[320px]">
        <div>
          ...
          <div className="max-w-lg mt-16 max-lg:hidden">
            <h3 className="text-3xl font-bold text-white">Member Login</h3>
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-500 sm:px-6 px-4 py-8 max-w-md w-full h-max shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] max-lg:mx-auto">
          <form>
            ...
            <div className="mb-8">
              <h3 className="text-3xl font-extrabold text-gray-80">Member Login</h3>
            </div>
  ...
  <select onChange={handleChange} value={role} name="role" className="select select-bordered">
    ...
    <option>jsec</option>
            ...
            <p className="text-sm mt-8 text-center text-gray-80">Are You Admin !?                   <Link href={`${process.env.NEXT_PUBLIC_HOST}/admin/login`} className="text-blue-600 font-semibold hover:underline ml-1 whitespace-nowrap">Login Here</Link></p>
          </form>
        </div>
      </div>
    </div>
